...
type Context = {
    cancelCapture: () => Promise<{success: boolean} | undefined>;
    feature: Feature;
...
export const useCancelCapture = () => {
    ...
    const {call, feature, responseError} = useICCanisterCallContract('cancelCaptureIdentity');

    const cancelCapture = useMemo(
        () =>
            reusePromiseWrapper(
                async () => {
                    const logMessagePrefix = `useCancelCapture:`;
    ...
    return useMemo<Context>(() => ({cancelCapture, feature, responseError}), [cancelCapture, feature, responseError]);
};
