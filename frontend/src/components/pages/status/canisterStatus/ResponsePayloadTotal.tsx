...
export const ResponsePayloadTotal = () => {
    ...
    const responsePayloadTotal = useMemo(
        () => convertBytesFractionalAdaptive(data?.canister_status_response.query_stats.response_payload_bytes_total),
        [data?.canister_status_response.query_stats.response_payload_bytes_total]
    );

    if (isNullish(responsePayloadTotal) || nonNullish(responseError) || canisterStatusFeature.error.isError) {
        return <LoadingAndFailedToLoadValueWrapper loaded={loaded} isError inProgress={inProgress} action={fetchCanisterStatus} />;
    }
    return formatMemoryBytesValueWithUnit(responsePayloadTotal);
};
