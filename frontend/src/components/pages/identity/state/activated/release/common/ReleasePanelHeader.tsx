...
export const ReleasePanelHeader = () => {
    const {identityNumber} = useIdentityHolderContextSafe();
    const message = useMemo(() => i18.holder.state.release.common.panelTitle(identityNumber), [identityNumber]);
    return (
        <Flex justify="space-between" align="center">
            <PanelHeader title={message} />
            <ExternalLinkToFAQAsQuestionMark fragment="transfer-from" />
        </Flex>
    );
};
