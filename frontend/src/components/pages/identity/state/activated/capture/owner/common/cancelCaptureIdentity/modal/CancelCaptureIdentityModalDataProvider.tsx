...
export const useCancelCaptureIdentityModalDataContext = () => {
    const context = useContext<Context | undefined>(Context);
    if (!context) {
        throw new Error('useCancelCaptureIdentityModalDataContext must be used within a CancelCaptureIdentityModalDataProvider');
    }
    return context;
};
...
export const CancelCaptureIdentityModalDataProvider = (props: PropsWithChildren<Props>) => {
    const {onCancelModal} = props;
    const identityHolderLoaded = useRefetchIdentityHolder();
    const {
        cancelCapture: {cancelCapture, feature, responseError},
        ownerCanCancelCapture: {ownerCanCancelCapture}
    } = useCancelCaptureIdentityDataContext();
    ...
    */
    ...
    const actionAvailability = useMemo<ActionAvailability>(() => {
        ...
        if (!ownerCanCancelCapture) {
            return {type: 'notAvailable'};
        }
        return {type: 'available'};
    }, [identityHolderLoaded, ownerCanCancelCapture]);
    ...
    */
    ...
    }, [actionAvailability.type, cancelCapture]);
    ...
    */
    ...
    const okButtonProps: ButtonProps = useMemo(() => {
        ...
        result.children = i18.holder.state.capture.common.modal.restartTransfer.button;
