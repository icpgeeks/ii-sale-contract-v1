...
pub(crate) async fn process(
    ...
    log_info!(env, "Hub reward: transferring ...");
    ...
    let transfer_amount =
        rewards_calculator.calculate_hub_reward_transfer_amount(transit_account_balance);

    match transfer_amount {
        Err(reason) => {
            log_info!(
                env,
                "Hub reward: failed to calculate transfer amount, reason: {reason}."
            );
            transfer_completed(lock, None)
        }
        Ok(None) => {
            log_info!(env, "Hub reward: transfer already completed.");
            transfer_completed(lock, None)
        }
        Ok(Some(transfer_amount)) => {
            let receiver_account = env.get_settings().hub_reward_account;
            ...
            log_info!(
                env,
                "Hub reward: transfer completed, info: {transfer_information:?}."
            );
    ...
    update_holder(
        ...
        HolderProcessingEvent::Holding {
            event: HoldingProcessingEvent::SaleDeal {
                event: Box::new(SaleDealProcessingEvent::HubRewardTransferred { transfer }),
            },
        },
    )
